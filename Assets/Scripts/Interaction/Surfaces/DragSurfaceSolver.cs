...
public sealed class DragSurfaceSolver
{
    ...
    /// <summary>
    /// Создаёт решатель для заданного корня и опционального MeshFilter поверхности.
    /// </summary>
    /// <param name="planeRoot">Трансформ плоскости; не null.</param>
    /// <param name="planeMeshFilter">MeshFilter поверхности для клампа или null.</param>
    public DragSurfaceSolver(Transform planeRoot, MeshFilter planeMeshFilter)
    {
        _root = planeRoot;

        if (planeMeshFilter != null && planeMeshFilter.sharedMesh != null)
        {
            _meshLocalBounds = planeMeshFilter.sharedMesh.bounds;
            _clampToMesh = true;
        }
        else
        {
