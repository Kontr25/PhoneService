...
namespace UI.Attributes
{
    ...
    public class PropertyFoldoutGroupAttribute : Sirenix.OdinInspector.FoldoutGroupAttribute
    {
        ...
        /// <param name="_order">The order of the group in the inspector.</param>
        public PropertyFoldoutGroupAttribute(string _groupName, bool _expanded, int _order = 0): base(_groupName, _expanded, _order) { }
    }
    ...
    public class PropertyFoldoutGroupAttributeDrawer: OdinGroupDrawer<PropertyFoldoutGroupAttribute>
    {
        ...
        protected override void Initialize()
        {
            ...
            if (HeaderProperty == null)
                ErrorMessage = "No property or field named " + propName + " found. Make sure the property is part of the inspector and the group.";
            else
            {
                TitleHelper = null;
                ...
                if (Attribute.Title != null)
                {
                    TitleHelper = Sirenix.OdinInspector.Editor.ValueResolvers.ValueResolver.Get<string>(Property, Attribute.Title);
                }
                else
                if (helperProperty.GetAttribute<HideLabelAttribute>() == null)
                {
                    LabelTextAttribute customLabel = helperProperty.GetAttribute<LabelTextAttribute>();
                    if (customLabel == null)
                        TitleHelper = Sirenix.OdinInspector.Editor.ValueResolvers.ValueResolver.Get<string>(Property, helperProperty.NiceName, helperProperty.NiceName);
                    else
                        TitleHelper = Sirenix.OdinInspector.Editor.ValueResolvers.ValueResolver.Get<string>(Property, customLabel.Text, customLabel.Text);
                }
#endif
            }
        ...
        protected override void DrawPropertyLayout(GUIContent _label)
        {
            ...
            else
            {
                ...
                var foldoutRect = EditorGUILayout.GetControlRect(false, GUILayout.Width(_label != null ? EditorGUIUtility.labelWidth : 10f));
